...
router.get("/api/internal-account-statement", async (req, res) => {
  ...
  try {
    ...
    const [openingRows] = await connection.execute(
      `SELECT balance FROM to_and_from_transaction
       WHERE account_code = ? AND DATE(transaction_date) < DATE(?)
       ORDER BY transaction_date DESC, id DESC LIMIT 1`,
      [accountCode, fromDate]
    );
    const openingBalance = openingRows.length > 0 ? parseFloat(openingRows[0].balance) : 0;
...
router.get('/api/teller-statement', async (req, res) => {
  ...
  try {
    ...
    // -----------------------------------------------------------------
    // 1. Opening balance (most recent before fromDate from both tables)
    // -----------------------------------------------------------------
    const [openingRows] = await connection.execute(
      `SELECT balance FROM (
        SELECT balance, transaction_date, id 
        FROM deposit_to_and_from_transaction 
        WHERE account_name = ? AND DATE(transaction_date) < DATE(?)
        UNION ALL
        SELECT balance, transaction_date, id 
        FROM to_and_from_transaction 
        WHERE account_name = ? AND DATE(transaction_date) < DATE(?)
      ...
      LIMIT 1`,
      [accountNameUsed, fromDate, accountNameUsed, fromDate]
    );
    const openingBalance = openingRows.length > 0 ? parseFloat(openingRows[0].balance) : 0;

    // -----------------------------------------------------------------
    // 2. Currency (most recent transaction from either table)
    // -----------------------------------------------------------------
    const [currencyRows] = await connection.execute(
      `SELECT currency FROM (
        SELECT currency, transaction_date, id 
        FROM deposit_to_and_from_transaction 
        WHERE account_name = ?
        UNION ALL
        SELECT currency, transaction_date, id 
    ...
    const currency = currencyRows.length > 0 ? currencyRows[0].currency : 'GHS';

    // -----------------------------------------------------------------
    // 3. Transactions within date range – UNION with NULL for missing account_code
    // -----------------------------------------------------------------
    const [transactions] = await connection.execute(
      `SELECT
        id,
        ...
        created_at AS transactionDateTime,
        account_name,
        NULL AS account_code,        -- deposit table lacks account_code
        narration,
        description,
        debit,
        credit,
        balance,
        currency,
        'deposit_to_and_from' AS source_table
      FROM deposit_to_and_from_transaction
      WHERE account_name = ? AND DATE(transaction_date) BETWEEN DATE(?) AND DATE(?)
      ...
      SELECT
        ...
        created_at AS transactionDateTime,
        account_name,
        account_code,                -- to_and_from has this column
        narration,
        description,
        debit,
        credit,
    ...
    res.json({
      ...
      data: {
        ...
        transactions: transactions.map(t => ({
          ...
          credit: parseFloat(t.credit) || 0,
          balance: parseFloat(t.balance) || 0,
          account_code: t.account_code || '', // ensure it's always a string
        }))
      }
    });
...
router.get('/api/teller-callover', async (req, res) => {
  const { accountName, fromDate, toDate, type } = req.query;
  ...
  try {
    ...
    // -----------------------------------------------------------------
    // 3. Build type condition (deposit = credit only, withdrawal = debit only)
    // -----------------------------------------------------------------
    let typeCondition = '';
    if (type === 'deposit') {
      typeCondition = 'AND credit > 0 AND debit = 0';
    ...
    // -----------------------------------------------------------------
    // 4. Get transactions within date range from both tables
    //    – skip account_number / account_code to avoid column mismatch
    // -----------------------------------------------------------------
    const [transactions] = await connection.execute(
      `SELECT
        id,
    ...
    connection.release();
    console.error("Teller Callover Error:", error);
    res.status(500).json({
      success: false,
      message: "Failed to generate call-over statement",
      error: error.message
    });
  }
});
