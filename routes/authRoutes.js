    ...
    db.query(checkUserSql, [email, cleanPhone], async (err, results) => {
      ...
      db.query(
        ...
        (err) => {
          ...
          res
            .status(201)
            .json({ message: "Account created successfully!", role: userRole });
        },
      );
    });
  } catch (err) {
  ...
  try {
    ...
    if (userRole === "teller") {
      ...
      if (rows.length === 0) {
        ...
        const lastNumber = parseInt(rows[0].teller_id.replace("Till", ""));
...
router.post("/api/change-staff-password", async (req, res) => {
  ...
  try {
    const [rows] = await db
      .promise()
      .query("SELECT password FROM users1 WHERE userId = ?", [userId]);
    ...
    await db
      .promise()
      .query("UPDATE users1 SET password = ? WHERE userId = ?", [
        hashedPassword,
        userId,
      ]);
