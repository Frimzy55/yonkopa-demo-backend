...
app.post('/signup2', async (req, res) => {
  ...
  console.log('Received data:', { name, username, identifier, role });
  ...
  try {
    ...
    db.query(
      ...
      [
        name.trim(),
        username.trim().toLowerCase(),   // ✅ store normalized
        email,
        phone,
        hashedPassword,
        userRole
      ...
      (err, result) => {
        if (err) {
          ...
          // duplicate username error (optional friendly message)
  ...
  try {
    ...
    db.query(sql, [isEmail ? identifier : phone], async (err, results) => {
      ...
      if (results.length === 0) {
        ...
        // optional log (unknown user attempt)
        return res.status(404).json({ message: 'User not found' });
      }
...
app.get('/getusers', (req, res) => {
  const sql = `
    SELECT userId, full_name, username, email, phone, role, status,created_at
    FROM users1
...
app.put('/users/:id', (req, res) => {
  ...
  if (identifier) {
    ...
    if (isEmail) {
      ...
      phone = cleanIdentifier.replace(/\D/g, '');
    }
  }
  ...
  db.query(
    sql,
    [fullName, username, email, phone, role, id],
    (err) => {
      if (err) {
...
app.get("/loan-check/:userId", (req, res) => {
  const { userId } = req.params;
...
app.post("/assign-tasks", (req, res) => {
  ...
  // STEP 1: remove old tasks first
  const deleteSql = "DELETE FROM tasks WHERE userId = ?";

  db.query(deleteSql, [userId], (deleteErr) => {
    if (deleteErr) {
      console.error(deleteErr);
      return res.status(500).json({ message: "Failed to clear old tasks" });
    }

    // STEP 2: insert fresh tasks
    if (tasks.length === 0) {
      return res.json({ message: "Tasks cleared successfully" });
    }
...
app.delete("/remove-task", authenticateToken, (req, res) => {
...
app.put('/deactivate-user/:id', (req, res) => {
  const { id } = req.params;
